...
const SidebarTrigger = React.forwardRef<React.ElementRef<typeof Button>, React.ComponentProps<typeof Button>>(({ className, onClick, children, ...props }, ref) => {
  ...
  // Mobile: simple button with children (Menu icon)
  if (isMobile) {
    return (
      <Button
        ref={ref}
  ...
  // Desktop: collapse/expand button with arrows
  const button = (
    <Button
      ref={ref}
      data-sidebar="trigger"
      variant="ghost"
      size="default"
      className={cn("h-8 w-full justify-start gap-2 group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 group-data-[collapsible=icon]:justify-center", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
...
const SidebarFooter = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => {
  return <div ref={ref} data-sidebar="footer" className={cn("flex flex-col gap-2 p-2 group-data-[collapsible=icon]:p-2 group-data-[collapsible=icon]:items-center", className)} {...props} />;
});
